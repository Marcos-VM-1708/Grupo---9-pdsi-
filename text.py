...
def get_midi_path(root):
    ...
    for dirpath, _, filenames in os.walk(root):
        for filename in filenames:
            if filename.lower().endswith(('.mid', '.midi')):  # Fix file extension check
...
def main():
    ...
    jazz_paths = get_midi_path(jazz_folder)
    pop_paths = get_midi_path(pop_folder)

    # Combine paths from both jazz and pop folders
    midi_paths = jazz_paths
